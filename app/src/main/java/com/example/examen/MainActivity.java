...
import com.example.examen.ml.ModelUnq;
...
public class MainActivity extends AppCompatActivity implements OnSuccessListener<Text>,
    ...
    public void PersonalizedModel(View v){
        try {
            ...
            String[] etiquetas ={"LABORATORIO AGROINDUSTRIAL","CENTRO MEDICO","BIBLIOTECA","LABORATORIO INDUSTRIAL","POLIDEPORTIVO","FACULTAD FCI","FACULTAD CAYF","PARQUEADERO","BAÑOS","LABORATORIO ACUICULTURA","COMEDOR","FACULTAD FCIP"};

            ModelUnq model = ModelUnq.newInstance(getApplicationContext());
            TensorBuffer inputFeature0 = TensorBuffer.createFixedSize(new int[]{1, 224, 224, 3}, DataType.FLOAT32);
            inputFeature0.loadBuffer(convertirImagenATensorBuffer(mSelectedImage));

            ModelUnq.Outputs outputs = model.process(inputFeature0);
            TensorBuffer outputFeature0 = outputs.getOutputFeature0AsTensorBuffer();

            txtResult.setText(obtenerEtiquetayProbabilidad(etiquetas, outputFeature0.getFloatArray()));
